...
@Slf4j
public class RemoveFollowThread extends Thread{

    Follow follow;

    public RemoveFollowThread(Follow follow) {
        this.follow = follow;
    }
    ...
    public void run() {
        ...
        try{
            status = userRepository.rmFollow(follow);
            session.commit();
        }catch (Exception e) {
            log.error("[关注信息更新失败] business_id = {}, openid = {}", follow.getBusiness_id(), follow.getOpenid());
        }
        if(status != -1) {
            log.info("[关注信息已已更新] business_id = {}, openid = {}", follow.getBusiness_id(), follow.getOpenid());
        }else {
            log.error("[关注信息更新失败] business_id = {}, openid = {}", follow.getBusiness_id(), follow.getOpenid());
        }
    }
